...
class Sock5Proxy:
    ...
    def decorate_socket(self, s):
      if self.username and self.password:
        s.set_proxy(socks.PROXY_TYPE_SOCKS5, self.host, self.port, self.rdns, self.username, self.password)
      else:
        s.set_proxy(socks.PROXY_TYPE_SOCKS5, self.host, self.port, self.rdns)
      return s
